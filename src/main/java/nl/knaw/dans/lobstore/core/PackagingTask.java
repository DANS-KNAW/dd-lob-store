...
public class PackagingTask implements Runnable {
    ...
    public void run() {
        try {
            ...
            executePackagingCommand(bucketName);
    ...
    private void executePackagingCommand(String bucketName) throws IOException {
        CommandLine commandLine = new CommandLine(packagingCommand.getExecutable());
        for (String arg : packagingCommand.getArgs()) {
            commandLine.addArgument(arg.replace("${bucketname}", bucketName));
        }

        log.debug("Executing packaging command: {}", commandLine);

        var builder = new Builder<>();
        if (packagingCommand.getWorkingDirectory() != null) {
            String wd = packagingCommand.getWorkingDirectory().replace("${bucketname}", bucketName);
