...
folders are assigned a quota, managed by a disk quota manager. The idea is that a task must first claim sufficient disk space to perform its work. The steps
below, starting from [Inspect](#inspect), all use a polling mechanism, which checks the source table for new work. Before scheduling a task, the service checks
if the necessary disk space can be claimed. If not, the task is not scheduled and will be retried in the next polling cycle.

### Request

The service receives requests for transfer via the API. It checks whether the SHA-1 checksum is already present in the targeted LOB-store. If so, the transfer
request is immediately changed to `DONE`. It also checks whether a transfer request for the same SHA-1 is already present with a status other than `FAILED`,
`REJECTED` or `DONE`. If so, the transfer request is not created and a 409 Conflict status is returned to the caller.
...
Finally, the status of all the corresponding transfer requests is set to `PACKAGED`.
